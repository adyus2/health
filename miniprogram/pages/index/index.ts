...
import { TAB_INDEX, WEEK_LABELS } from '../../config/constants';
const TIPS = [
...
Page({
  ...
  onShow() {
    ...
    wx.nextTick(() => {
      ...
      if (tabBar && tabBar.data.selected !== TAB_INDEX.HOME) tabBar.setData({ selected: TAB_INDEX.HOME });
    });
  },
    ...
    try {
      ...
      if (remote && remote.nickname) {
        this.setData({
          ...
          userEmoji: remote.gender === '男' ? '👨' : remote.gender === '女' ? '👩' : '🧑',
        });
      }
    } catch (err) {
      // 远程失败时退回到本地缓存
