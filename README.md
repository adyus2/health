...
- 修复跨域 CORS 预检请求（OPTIONS）被鉴权中间件拦截的问题，解决前端无法调通后端接口
- 修复 `AUTH_SECRET` 环境变量缺失导致服务启动失败，增加失败日志提示
- 修复 SQLite 数据库目录不存在时服务崩溃，增加自动创建目录逻辑
- 新增 `.env` 文件与 `project.private.config.json` 到 `.gitignore`，避免敏感信息误提交
- 修复 `history` 页面日期格式处理逻辑，支持登录后自动请求月度数据
- 增加 `/api/ping` 健康检查路由，方便验证服务是否正常

## 文档索引

- 改进记录：docs/DEVELOPMENT_UPDATE_LOG.md
...
- 同步冲突处理和自动化测试尚未完整覆盖
- 后端 `server/node_modules/` 目录不应提交到 Git，本地自行 `npm install`
